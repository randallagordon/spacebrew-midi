...
var midiUtil = {};
midiUtil.humanizeNote = function( hex ) {
...
var handleMIDI = function( deltaTime, message ) {
  switch( true ) {
    case ( message[ 0 ] >= 0x80 && message[ 0 ] <= 0x8F ):
      console.log( 'Note off: ' + midiUtil.humanizeNote( message[ 1 ] ) );
      sb.send( message[ 1 ].toString(), 'boolean', 'false' );
      break;
    case ( message[ 0 ] >= 0x90 && message[ 0 ] <= 0x9F ):
      console.log( 'Note on: ' + midiUtil.humanizeNote( message[ 1 ] ) + ' with velocity 0x' + message[ 2 ].toString( 16 ) );
